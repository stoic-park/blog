...
**결론**: 프론트엔드에서 할 수 있는 최적화는 거의 한계에 도달한 상태였다. 근본적으로 **브라우저가 보유해야 하는 데이터 총량 자체**를 줄여야 했다.
...
Raw chart에서는 하나의 wafer에 여러 sensor 데이터가 포함된다. 모든 센서가 동일한 timestamp를 공유하므로, **첫 번째 센서에서 LTTB 인덱스를 결정하고 동일 인덱스를 모든 센서와 시간축에 적용**한다. 이렇게 하면 센서 간 시간축 정렬이 유지된다.
...
BFF는 백엔드 원본 응답을 **단기 LRU 캐시**에 보관한다.
...
`data` 필드는 기존 백엔드 응답과 **100% 동일한 shape**이다. 프론트는 `data`만 사용하므로 기존 코드와 완벽 호환된다. `meta` 필드로 다운샘플링 효과를 실시간 모니터링할 수 있다.

---

## 자동 Fallback

운영 환경에서 가장 중요한 것은 **장애 시에도 서비스가 중단되지 않는 것**이다.
...
BFF가 다운되면 기존 direct 방식으로 자동 전환된다. 메모리는 높아지지만 **기능 중단은 없다.** 환경변수를 `direct`로 변경하면 BFF를 완전히 우회할 수도 있다.
...
| 항목 | Plotly Dash (레거시) | ECharts Direct (이전) | BFF + ECharts (현재) |
|---|---|---|---|
