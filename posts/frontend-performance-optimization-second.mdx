...
HTTP/1.1 환경에서는 차단 리소스를 줄이는 효과가 크지만, HTTP/2/3 환경에서는 멀티플렉싱 덕분에 효과가 줄어들고 캐시 전략이 중요합니다

결론적으로, 필수 `Above‑the‑fold` CSS만 인라인하고 나머지는 외부 CSS + preload로 처리하는 것이 가장 균형 잡힌 접근입니다.

### 3. 애니메이션 실행 최적화

웹 애니메이션은 **어느 렌더링 단계에서 실행되는가**에 따라 비용이 다릅니다.

- **Layout/Style**이 다시 계산되면 범위가 커지고 비용이 큽니다.
- **Paint**가 발생하면 픽셀을 다시 그려야 하므로 부담이 큽니다.
- **Composite(합성)** 단계에서만 처리되면, 기존 레이어를 GPU에서 이동/합성하므로 가장 저렴합니다.
  따라서 **`transform`·`opacity` 중심**으로 애니메이션을 구성하는 것이 기본 원칙입니다.

#### 적용 방법

1. **합성 단계 속성만 애니메이션**

- 위치/크기 변화는 `left/top/width/height` 대신 `transform: translate/scale`로 대체
- 투명도 변화는 `opacity` 사용

2. **레이어 승격은 ‘필요 시, 짧게’ (`will-change`)**

- 진입 직전(hover/focus/in-view) 등 **짧은 기간만** 힌트를 주고, 끝나면 제거

3. **FLIP 패턴으로 레이아웃 변화 애니메이션**

- First→Last 위치/크기를 측정하고, Invert(차이만큼 `transform`) 후 Play(애니메이트)
- 레이아웃 변경을 합성 단계 애니메이션으로 인지적으로 연결

4. **스크롤 연동은 CSS/WAAPI 우선**

- 가능하면 **CSS Scroll-Driven Animations(ScrollTimeline)** 또는 **IntersectionObserver + class 토글**을 사용
- JS 스크롤 핸들러 연속 호출은 INP/TBT 악영향 → 쓰로틀/오프로드/간헐 토글

5. **접근성 고려**

- 움직임 민감 사용자를 위해 `prefers-reduced-motion` 분기 제공

6. **React/Next.js 실무 팁**

- 프레임마다 `setState`로 애니메이션하지 말고 **CSS 전환** 또는 **WAAPI** 사용
- 대량 리스트의 항목 애니메이션은 **가상화 + 합성 속성** 조합
- 프레이머/애니메이션 라이브러리는 필요한 범위만 도입하고 동적 임포트로 분리
...
/* ❌ 비용 큰 속성 애니메이션 (paint/layout 유발)
.bad:hover { width: 320px; box-shadow: 0 0 24px #000; }
*/
```
...
```js
function flipMove(el, applyLayoutChange) {
 const first = el.getBoundingClientRect()
 applyLayoutChange() // 클래스 토글 등 레이아웃 변경
...
- **Chrome DevTools → Performance**: 애니메이션 구간에서 **Layout/Paint/Composite** 분포 확인
- **Rendering 패널**: _Paint flashing_ 켜서 repaint 영역 확인, *Layer borders*로 합성 레이어 시각화
- **Layers/Elements**: 문제 노드의 레이어 승격 여부와 스타일 변경 촉발 속성 파악
- **Web Vitals(INP/TBT)**: 입력 응답성 악화 시, 핸들러/JS 작업 분할 여부 점검

#### 트레이드오프 / 주의사항

- **레이어 남발 금지**: `will-change` 상시 적용은 GPU 메모리/합성 비용 증가
- **대량 요소 동시 애니메이션 지양**: 합성만 써도 레이어/명령 큐가 과부하
- **그림자/필터 장시간 애니메이션 주의**: paint 코스트 큼 → 필요 시 정적 이미지/레이어 분리로 대체
- **JS 스크롤 핸들러 남용 주의**: non-passive 리스너/연속 동기 작업은 INP/TBT 악화 → `passive: true`/쓰로틀/오프로드
- **React 상태 기반 프레임 업데이트 지양**: 클래스 토글·WAAPI·CSS 전환 우선

#### 정리

애니메이션은 가능한 \*\*합성 단계(`transform`·`opacity`)\*\*로 한정하고, **일시적 레이어 승격**과 \*\*플로우 보존(FLIP)\*\*으로 시각 자연스러움을 확보하세요.
DevTools로 **Layout/Paint/Composite** 비중을 확인해, paint·layout 스파이크가 보이면 속성 선택/설계 방식을 재검토하는 것이 정석입니다.
참고: [MDN – High performance animations](https://developer.mozilla.org/en-US/docs/Web/CSS/will-change), [MDN – will-change](https://developer.mozilla.org/en-US/docs/Web/CSS/will-change), [web.dev – Stick to compositor‑only properties](https://web.dev/animations-guide/).
