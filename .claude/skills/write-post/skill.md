...
**주의:** Table 셀 내부는 일반 텍스트로 렌더링되므로 `**bold**`나 마크다운 문법이 적용되지 않습니다. 강조가 필요하면 셀 밖 본문에서 처리.

### 도메인 민감 내용 필터링
회사명/프로젝트명/내부 시스템명/내부 도메인 용어 직접 노출 금지.
소스 문서의 내부 식별자(예: 내부 페이지명, 도메인 약어)는 반드시 일반화.
...
- 본문 첫 이미지가 자동으로 썸네일 (없으면 OG 라우트 자동 생성)
